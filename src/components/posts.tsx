              ...
              {postsByYear[Number(year)]
                ...
                .map(post => (
                  <li key={post.slug} className="mb-1">
                    <Link
                      className="flex flex-row items-center space-x-2 py-1 px-2 rounded-md lg:hover:outline-dashed lg:hover:outline-muted lg:hover:outline-[0.2em] hover:text-primary w-8/12"
                      href={`/blog/${post.slug}`}
                    >
